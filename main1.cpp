...
int main()
{
    img = imread("images/startpos2.png", 1);
 ...
 Arr[0].state=Arr[1].state=Arr[2].state=Arr[3].state=Arr[4].state=Arr[5].state=2;
 Arr[6].state=Arr[7].state=Arr[8].state=Arr[9].state=Arr[10].state=Arr[11].state=1;
 Arr[0].count=Arr[1].count=Arr[6].count=Arr[7].count=1;
 Arr[2].count=Arr[3].count=Arr[4].count=Arr[8].count=Arr[9].count=Arr[10].count=2;
Arr[5].count=Arr[11].count=8;
namedWindow("you",CV_WINDOW_NORMAL);
	...
	for(i=0;i<8;i++){
		for(j=0;j<8;j++){
			cout<<board[i][j].ltop.first<<","<<board[i][j].ltop.second<<endl;
			cout<<board[i][j].rbottom.first<<","<<board[i][j].rbottom.second<<endl;
		}
	}
	...
	//imshow("point detection part",src);
	 //img = imread("images/chesscropped.PNG",1);
	 img= croppedImage;

	 /*imshow("yuo",img);
	 waitKey(0);*/
 
 Mat img2(img.rows, img.cols, CV_8UC1, Scalar(130));
...
imshow("Cropped portion1",result);
...
*/
	...
	for (i = 0; i < img.rows; i++)
	{
	    for (j = 0; j < img.cols; j++)
		{             
                   if(((img.at<Vec3b>(i, j)[2]+img.at<Vec3b>(i, j)[1]+img.at<Vec3b>(i, j)[0])/3>230)&&((img.at<Vec3b>(i, j)[2]+img.at<Vec3b>(i, j)[1]+img.at<Vec3b>(i, j)[0])/3<253))
                     img2.at<uchar>(i,j)=255;
                 	else if ((img.at<Vec3b>(i, j)[2]+img.at<Vec3b>(i, j)[1]+img.at<Vec3b>(i, j)[0])/3<120)
                 	{
                 		img2.at<uchar>(i,j)=0;
                 	}
                   else 
                     img2.at<uchar>(i,j)=125;
                  }
  	}
  	imshow("Cropped portion",img);
  	imshow("you",img2);
	waitKey(0);
  	...
  	for(i=0;i<12;i++){
		...
		resize(Arr[i].img, out, Size(98,98), 98/Arr[i].img.cols, 98/Arr[i].img.rows);
		Arr[i].img = out;
		
		resize(Arr[i].img1, out, Size(98,98),98/Arr[i].img1.cols, 98/Arr[i].img1.rows);
		Arr[i].img1 = out;
	}

  	cout<<Arr[0].img.rows<<","<<Arr[0].img.cols<<endl;
  	cout<<Arr[1].img.rows<<","<<Arr[1].img.cols<<endl;
  ...
  //Mat Arr[k].img(Arr[k].img.rows, Arr[k].img.cols, CV_8UC1, Scalar(0));
	...
	for (i = 0; i < Arr[k].img.rows; i++)
	{
	    for (j = 0; j < Arr[k].img.cols; j++)
		{     
                   if(((Arr[k].img.at<Vec3b>(i, j)[2]+Arr[k].img.at<Vec3b>(i, j)[1]+Arr[k].img.at<Vec3b>(i, j)[0])/3>230)&&((Arr[k].img.at<Vec3b>(i, j)[2]+Arr[k].img.at<Vec3b>(i, j)[1]+Arr[k].img.at<Vec3b>(i, j)[0])/3)<253)
                     Arr[k].img1.at<uchar>(i,j)=255;
                   else if ((Arr[k].img.at<Vec3b>(i, j)[2]+Arr[k].img.at<Vec3b>(i, j)[1]+Arr[k].img.at<Vec3b>(i, j)[0])/3<120)
                   {
                   	 Arr[k].img1.at<uchar>(i,j)=0;
                   }
                   else
                     Arr[k].img1.at<uchar>(i,j)=125;
...
for(i=0;i<12;i++)
  ...
  Point matchLoc;
  /// Create the result matrix
  int result_cols =  img2.cols - Arr[i].img1.cols + 1;
  int result_rows = img2.rows - Arr[i].img1.rows + 1;
 Mat result(result_rows,result_cols,CV_8UC1,Scalar(0));
  /// Do the Matching and Normalize
 	//cout<<Arr[i].img1.rows<<endl;
    matchTemplate( img2,Arr[i].img1, result, CV_TM_SQDIFF );
...
//	cout<<"yo2"<<endl;
  ...
  rectangle( img2, matchLoc, Point( matchLoc.x + Arr[i].img1.cols , matchLoc.y + Arr[i].img1.rows ), Scalar::all(0), CV_FILLED, 8, 0 );
  
  imshow("you",img2);
  waitKey(0);

  cout<<i<<","<<matchLoc.x<<","<<matchLoc.y<<endl;
  for(x=0;x<8;x++)
  {
    for(y=0;y<8;y++)
    {
      if(abs(board[x][y].ltop.first-matchLoc.x)<30 && abs(board[x][y].ltop.second-matchLoc.y)<30)
      {
  ...
  cout<<Arr[10].piece<<","<<Arr[10].state<<endl;
   for(x=0;x<8;x++)
  {
    for(y=0;y<8;y++)
    {
